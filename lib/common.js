...
function initClearCookie(intervalTime) {
    window.setInterval(clearAllBrowserData, intervalTime);
}
...
chrome.storage.local.get(prefs, ps => {
    ...
    let timeoutInit = intervalTime - ((new Date()).getTime())%(ps.clear_browser_data_mm * 60 * 1000);
    window.setTimeout(initClearCookie(intervalTime), timeoutInit);
});