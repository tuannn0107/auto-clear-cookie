...
function clearAllBrowserData() {
    ...
    chrome.storage.local.set(extensionId + '_last_execution', (new Date()).getTime());
...
chrome.runtime.onMessage.addListener(request => {
    if (request.method === 'request-update') {
        ...
        let time = chrome.storage.local.get(extensionId + '_next_execution');
        if (time === undefined)
        {
...
chrome.storage.local.get(prefs, ps => {
    ...
    chrome.storage.local.set(extensionId + '_next_execution', (new Date()).getTime())
